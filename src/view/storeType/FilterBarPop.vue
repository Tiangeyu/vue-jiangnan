        ...
        <div class="main">
          <div class="main-sidebar" :class="{'full-line': !items,'bg-style':items,'line-style':!items,}">
            <div v-if="menu.type !== 'filter'" class="item" @click="clickSidebar(sidemenu, index)" v-for="(sidemenu, index) in sideMenus.detailList"
              ...
              <span class="downArea">{{ sidemenu.name }}</span>
            </div>
...
<script>
  export default {
    ...
    created (){
       this.$http.get("/eb-web-api/mobile/store/querySecondeCategory").then((res)=>{
          this.secondList = res.body.listCategory;
  ...
  .filterbarpop-wrap {
    ...
    .filterbarpop {
      ...
      .tab-bar {
        ...
        height: 40px;
        .selected {
          border-bottom: 2px solid #3da4f1;
